...
6. Based on your results from question 5, for which value of `hsize` would be predict the *highest* SAT score?
7. Augment your regression from question 5 by addint `hsperc`, `sat`, `female`, and `athlete` and display the results. Do you find strong evidence that adding these variables improves our predictions? Carry out appropriate statistical inference to answer this question.
...
10. Add `sat` back into the model, but now allow the effect of `athlete` to vary by sex. Do you find evidence that we should predict difference GPAs for female athletes versus female non-athletes? 
11. To improve our predictions, does it make sense to allow the effect of `sat` on GPA to differ by sex? Justify your answer.
12. Based on your results from this question, about how accurately does our best model predict GPA?
...
2. Use your function to plot the K-nearest neighbors regression curve to predict `mpg` from `dist` in the `mtcars` dataset (this is a built-in R dataset).
