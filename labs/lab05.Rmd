...
A tibble *is* a dataframe even though a datafram is not a tibble.
...
x <- rnorm(n)
z <- rnorm(n)
reg3 <- lm(mpg ~ disp + hp + wt + cyl + x + z, mtcars)
linearHypothesis(reg3, c('x = 0', 'z = 0'))
```
<!-- ANS_END -->
...
<!-- ANS_START -->
```{r}
reg4 <- lm(mpg ~ disp + I(disp^2) + I(disp^3), mtcars)
ggplot(mtcars, aes(x = disp, y = mpg)) +
  geom_point() +
...
<!-- ANS_START -->
```{r}
reg5 <- lm(mpg ~ disp + wt + I(disp^2) + I(wt^2) + disp:wt, mtcars)
coef(reg5)
```
<!-- ANS_END -->
...
2. Use the following:
```{r}
mydat <- data.frame(disp = c(125, 175, 225))
predict(reg4, mydat)
```
<!-- ANS_END -->
