...
avg_dist <- wells %>% 
  ...
  pull(avg_dist)
mean_pred1 <- predict(fit1, newdata = data.frame(dist100 = avg_dist), type = 'response')
mean_pred1
...
mean_household <- data.frame(dist100 = avg_dist, 
                             ...
                             zeduc = 0)
mean_pred4 <- predict(fit4, newdata = mean_household, type = 'response')
coef(fit4)[-1] * lambda(mean_pred4)
```
<!-- ANS_END -->
