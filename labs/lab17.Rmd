...
\alpha_0 + \alpha_1 X_i  + \epsilon, & \mbox{for } X_i < c\\
\beta_0 + \beta_1 X_i  + \epsilon, & \mbox{for } X_i \geq c\\
\end{array}
\right.
\]
...
&= \alpha_0 + (\beta_0 - \alpha_0) D_i + \alpha_1 X_i + (\beta_1 - \alpha_1) D_i X_i + \epsilon_i
\end{align*}
Thus, we see that $\gamma_0 = \alpha_0, \gamma_1 = (\beta_0 - \alpha_0), \gamma_2 = \alpha_1$, and $\gamma_3 = (\beta_1 - \alpha_1)$.
3. The RD causal effect is $(\beta_0 - \alpha_0) + (\beta_1 - \alpha_1)c = \gamma_1 + \gamma_3 c$.
4. Replacing $X$ with $(X - c + c)$, we obtain
\begin{align*}
Y_i &= \gamma_0 + \gamma_1 D_i + \gamma_2 (X_i - c + c) + \gamma_3 D_i (X_i - c + c) + \epsilon_i \\
&= \gamma_0 + \gamma_1 D_i + (\gamma_2 \widetilde{X}_i + \gamma_2 c) + (\gamma_3 D_i \widetilde{X}_i + \gamma_3 c D_i) + \epsilon_i \\
&= (\gamma_0 + c\gamma_2) + (\gamma_1 + c\gamma_3) D_i + \gamma_2 \widetilde{X}_i + \gamma_3 D_i \widetilde{X}_i + \epsilon_i \\
\end{align*}
...
# Regression line before cutoff 
xbefore <- seq(0, cutoff - 0.01, 0.01)
ybefore <- predict(rd, newdata = data.frame(xtilde = xbefore - cutoff,
                                         D = 1 * (xbefore >= cutoff)))
points(xbefore, ybefore, type = 'l', col = 'blue', lwd = 2)

# Plot regression line after cutoff
xafter <- seq(cutoff, 1, 0.01)
yafter <- predict(rd, newdata = data.frame(xtilde = xafter - cutoff,
                                         D = 1 * (xafter >= cutoff)))
points(xafter, yafter, type = 'l', col = 'red', lwd = 2)

# Does the true causal effect lie in the 95% confidence interval?
RD_true <- (b0 - a0) + (b1 - a1) * cutoff
RD_true
summary(rd)
